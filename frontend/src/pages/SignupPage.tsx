...
function HeroBranding() {
  return (
    <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center p-12 overflow-hidden">
      ...
      <div className="absolute -bottom-10 -left-10 h-56 w-56 rounded-full bg-violet/20 blur-3xl" />
      ...
      <div className="relative z-10 text-center text-white animate-fade-up">
        ...
        <p className="mt-3 text-lg text-white/80 max-w-xs mx-auto leading-relaxed">
          Build your personal AI knowledge base in minutes.
        </p>

        <div className="mt-10 grid grid-cols-1 gap-3 text-left max-w-xs mx-auto">
          {[
            { n: "01", title: "Upload", desc: "Any PDF, DOCX, TXT or Markdown file" },
            { n: "02", title: "Ask",    desc: "Natural language questions" },
            { n: "03", title: "Cite",   desc: "Answers grounded in your documents" },
          ].map((s) => (
            <div key={s.n} className="flex items-start gap-3 rounded-2xl bg-white/10 px-4 py-3 backdrop-blur-sm border border-white/10">
              <span className="mt-0.5 shrink-0 font-display text-xs font-bold text-white/40">{s.n}</span>
              <div>
    ...
    <div className="flex min-h-screen" style={{ background: 'linear-gradient(135deg, #0c1445 0%, #1e3a8a 40%, #2563eb 100%)' }}>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 lg:px-16 bg-white rounded-3xl shadow-xl m-4 lg:m-0 lg:rounded-none lg:rounded-l-3xl">
        ...
        <div className="w-full max-w-sm animate-fade-up">
          ...
          <p className="mt-1 text-sm text-t3">Free forever. No credit card required.</p>

          <form onSubmit={handleSubmit} className="mt-8 space-y-5" id="signup-form">
            {/* Full name */}
            <div>
              <label htmlFor="signup-name" className="mb-1.5 block text-xs font-semibold text-t2 uppercase tracking-wide">
                Full name <span className="normal-case font-normal text-t4">(optional)</span>
              </label>
              <input
                id="signup-name"
                type="text"
                ...
                className="w-full rounded-xl border border-t4 bg-white px-4 py-2.5 text-sm text-ink placeholder-t4 transition hover:border-accent/40 focus:border-accent"
              />
            </div>

            {/* Email */}
            <div>
              <label htmlFor="signup-email" className="mb-1.5 block text-xs font-semibold text-t2 uppercase tracking-wide">
                Email address
              </label>
              <input
                id="signup-email"
            ...
            {/* Password */}
            <div>
              <label htmlFor="signup-password" className="mb-1.5 block text-xs font-semibold text-t2 uppercase tracking-wide">
                Password
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-t4 bg-white px-4 py-2.5 pr-10 text-sm text-ink placeholder-t4 transition hover:border-accent/40 focus:border-accent"
                />
                <button
                  type="button"
              ...
              <p className="mt-1.5 text-xs text-t4">At least 8 characters</p>
            </div>
            ...
            <button
              ...
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-accent py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-accentDark hover:shadow-glow-lg disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading ? (
